...
GEM5_PATH=/fast_data/yonghae/AOS-gem5
SPEC_PATH=/fast_data/yonghae/benchmarks/spec2006_arm
SCRIPT_IN=$GEM5_PATH/configs/example/spec06_config.py
...
then
    SCHEME="AOS"
    OUTPUT_DIR=$GEM5_PATH/exp_script/output-AOS/SPEC-$BENCHMARK
else
    SCHEME="UnsafeBaseline"
    OUTPUT_DIR=$GEM5_PATH/exp_script/output/SPEC-$BENCHMARK
fi
...
CKPT_OUT_DIR=$GEM5_PATH/../gem5_ckpt_arm/$BENCHMARK-1-ref-arm
...
RUN_DIR=$SPEC_PATH/benchspec/CPU2006/$BENCHMARK_CODE/run/run_base_ref_x86_64_wllvm.0000
...
$GEM5_PATH/build/ARM_MESI_Two_Level/gem5.opt \
  ...
  --l1d_assoc=8 --l2_assoc=16 --l1i_assoc=4 --l1b_assoc=4 \
  --cpu-type=DerivO3CPU --scheme=$SCHEME --btNumWays=1 --MCQEntries=48 --BIBEntries=64 \
  --l2_size=8MB \
  --num-dirs=1 --ruby --maxinsts=3000000000 \
